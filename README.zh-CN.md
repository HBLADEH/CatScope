...
当前处于 **Preview** 阶段。发布版本与校验信息请以 GitHub Release 和 [变更记录](./CHANGELOG.md) 为准。
...
- [x] macOS universal preview 构建，支持 Intel 和 Apple Silicon。
- [ ] Linux 适配。
...
- Wails v2 CLI。
- Windows 与 Microsoft WebView2 Runtime，或用于当前 universal preview 的 macOS。
...
如需本地构建同时支持 Intel Mac 和 Apple Silicon Mac 的 universal preview 包：

```sh
scripts/build-macos.sh vX.Y.Z-preview
```

该脚本会执行 Go 测试、前端构建、`wails build -platform darwin/universal -clean -skipbindings`，生成 `dist/CatScope-v<version>-macos-universal.dmg` 及对应 `.sha256` 文件。发布时请显式传入 tag，例如 `scripts/build-macos.sh vX.Y.Z-preview`。

如果使用 GitHub Release 包，请下载 macOS DMG，打开后将 `CatScope.app` 拖入 Applications。当前 preview 版本是 self-signed/ad-hoc signed，尚未经过 Apple notarization，因此首次启动时 macOS Gatekeeper 可能要求通过 Finder 右键菜单打开，或在系统设置中允许该应用。

macOS 上 adb 通常叫 `adb`，不是 `adb.exe`。CatScope 会从 `ANDROID_HOME`、`ANDROID_SDK_ROOT` 和 `PATH` 自动检测 adb，但从 Finder 启动的 GUI 应用不一定继承 shell 的 `PATH`。如果检测失败，请在 CatScope 中配置完整 adb 路径，例如 `/Users/<you>/Library/Android/sdk/platform-tools/adb`。
...
优先平台: Windows
Preview 平台: macOS universal (Intel + Apple Silicon)
计划平台: Linux
```
...
使用、开发、QA 和发布资料已整理到 [文档索引](./docs/README.zh-CN.md)。常用入口：

- [用户指南](./docs/USER_GUIDE.zh-CN.md)
- [架构说明](./docs/ARCHITECTURE.md)
- [手工 QA 清单](./docs/QA_CHECKLIST.zh-CN.md)
- [发布与校验](./docs/RELEASE_ASSETS.md)
- [变更记录](./CHANGELOG.md)
...
欢迎提交 issue、建议和 pull request。完整的本地开发、验证和 PR 要求见 [贡献指南](./CONTRIBUTING.md)。当前项目仍处于早期阶段，高价值贡献方向包括：
...
CatScope 通过本机 adb 读取设备信息和实时 Logcat。离线日志文件只从本地磁盘读取。Workspace 和 preset 设置会以 JSON 保存在用户本地 CatScope 配置目录中，例如 Windows 下的 `%APPDATA%/CatScope/config.json` 或 macOS 下的 Wails 应用配置目录，不会写入 Android 项目目录。`.catscope-session` 文件是本地 JSON 文件，用于保存 CatScope 调试现场，包括日志、过滤条件、AnalysisResult 和 AI Context options；CatScope 不会将它们上传到云端。项目本身不要求上传日志到远程服务。AI Context Generator 只在本地生成适合复制或导出的 Markdown，不调用任何外部 AI API。分享导出的日志、session 文件或 AI Context 时，请注意不要公开敏感设备信息、用户数据、token、包名或内部业务日志。

## 许可证

CatScope 采用 [MIT 许可证](./LICENSE)，版权归 Copyright (c) 2026 HBLADEH 所有。

你可以自由使用、修改和再分发代码，包括用于商业项目，只需保留版权声明和许可证原文。软件按“原样”提供，不附带任何担保。
