...
CatScope 当前处于 MVP 阶段。核心 Logcat Viewer、Offline Log File Viewer、规则化 Crash / ANR / Native / JNI / Install Error Analyzer、本地 AI Context Generator、最小 Build / Install / Launch 工作流，以及轻量 Workspace / Filter Presets 已可用。
...
实时 Logcat 需要连接 Android 真机或模拟器，并在设备上允许 USB 调试授权。离线日志文件查看器不依赖 Android 设备，可打开 `.txt`、`.log` 或 `.jsonl` 文件并继续搜索、过滤、分析和生成 AI Context。
...
CatScope 通过本机 adb 读取设备信息和实时 Logcat。离线日志文件只从本地磁盘读取。Workspace 和 preset 设置会以 JSON 保存在用户本地 CatScope 配置目录中，例如 Windows 下的 `%APPDATA%/CatScope/config.json`，不会写入 Android 项目目录。项目本身不要求上传日志到远程服务。AI Context Generator 只在本地生成适合复制或导出的 Markdown，不调用任何外部 AI API。分享导出的日志或 AI Context 时，请注意不要公开敏感设备信息、用户数据、token、包名或内部业务日志。
