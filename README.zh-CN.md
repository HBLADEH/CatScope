...
CatScope 当前处于 MVP 阶段。核心 Logcat Viewer 与规则化 Crash / ANR / Native / JNI Analyzer 已可用。Build / Install / Launch 和 AI Context Generator 仍在规划或开发中。
...
- Install Error Analyzer: 安装错误识别、解释和建议。
- AI Context Generator: 生成适合 AI Agent 分析的 Markdown 上下文。
- 离线打开历史日志。
...
CatScope 聚焦 Logcat 周边的 Android 日常排障工作流。第一目标是做好 Logcat Viewer；构建、安装、启动、崩溃分析、AI 上下文是服务同一条调试链路的相邻能力。
...
2. 继续增强规则化 Analyzer，并补齐 AI 上下文生成。
3. 增加 Build / Install / Launch，形成基础日常调试闭环。
4. 逐步完善跨平台体验和历史日志分析。
...
CatScope 通过本机 adb 读取设备信息和 Logcat。项目本身不要求上传日志到远程服务。使用日志导出或后续 AI 上下文功能时，请注意不要公开敏感设备信息、用户数据、token、包名或内部业务日志。
