...
CatScope 当前处于 MVP 阶段。核心 Logcat Viewer、规则化 Crash / ANR / Native / JNI / Install Error Analyzer、本地 AI Context Generator，以及最小 Build / Install / Launch 工作流已可用。
...
CatScope 聚焦 Logcat 周边的 Android 日常排障工作流。第一目标是做好 Logcat Viewer；构建、安装、启动、崩溃分析、AI 上下文是服务同一条调试链路的相邻能力。当前 build runner 刻意保持轻量：它会执行默认 `assembleDebug` 等 Gradle wrapper 任务，但不是 Gradle Project Sync，也不是完整 IDE。
...
CatScope 通过本机 adb 读取设备信息和 Logcat。项目本身不要求上传日志到远程服务。AI Context Generator 只在本地生成适合复制或导出的 Markdown，不调用任何外部 AI API。分享导出的日志或 AI Context 时，请注意不要公开敏感设备信息、用户数据、token、包名或内部业务日志。
