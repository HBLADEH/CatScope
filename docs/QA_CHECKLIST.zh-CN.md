...
- [ ] 确认左侧显示的当前版本与 Release tag 一致。
- [ ] 关闭和开启“启动后自动检查”，重启后确认设置被保留。
- [ ] 正式版通道不应提示仅有的 Preview 更新；开启 Preview 后应能发现该版本。
- [ ] 没有新版本时，手动检查应明确提示当前已是最新版本。
- [ ] Windows 上用旧版 EXE 检测新 Release，确认下载、SHA256 校验、退出替换和重新启动完整成功。
- [ ] 在不可写目录或缺少对应 EXE / SHA256 资产时，确认失败可恢复，并可打开 Release 页面手动下载。
- [ ] macOS 上确认只提示更新并打开 Release 页面，不尝试直接替换 `.app`。
